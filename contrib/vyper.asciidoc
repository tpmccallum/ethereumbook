...
Vyper implements the logic of decorators explicitly. For example the Vyper code compilation process will fail if a function is preceeded with both the payable decorator and the constant decorators. Of course this makes complete sense because a constant function (one which only reads from the global state) will never need to accept value.
